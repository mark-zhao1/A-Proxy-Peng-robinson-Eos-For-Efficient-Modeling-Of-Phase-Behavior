...
Input
    T: Temperature [K]
    P: Pressure [bara]
    z: Overall composition. 1 x Nc array

    BIP: Binary interaction parameters. Nc x Nc array
    NRtol: Tolerance for Newton-Raphson root in Rachford-Rice.
    NRmaxit: Maximum number of iterations in NR Rachford-Rice.
    TolRR : Tolerance for Rachford-Rice determination of vapor fraction.
    SStol: Tolerance for successive-substitution (SS) in phase split.
    SSmaxit: Maximum number of iterations in SS.
    tolSSSA: Tolerance for residual of stationarity eqn in stability analysis (SA).
    TolXz : Tolerance to determine trivial solution in SA.
    itSSSAmax: Maximum number of iterations of SS in SA. Used in caseid2 to determine if diverged.

Ouput
    Number of phases/
    Composition of phases if not single phase.
